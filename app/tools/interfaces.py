...
async def sql_lookup(tool_input: ToolInput) -> ToolResult:
    ...
    # very small deterministic NL->SQL mapper for local data lookup
    mapping = {
    ...
    if not sql:
        return _empty(tool_name)
    try:
        await asyncio.wait_for(asyncio.sleep(0.02), timeout=tool_input.timeout_s)
    except TimeoutError:
        return _timeout(tool_name)
    return ToolResult(ok=True, tool_name=tool_name, data={"sql": sql, "rows": []})
